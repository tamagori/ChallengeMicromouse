...
void int_cmt0(void)
{
	...
	*****************************************************************************************/
	...
	if(run_mode == STRAIGHT_MODE){
		...
		//角加速度更新
		tar_ang_vel += ang_acc/1000.F;	//目標角速度を設定加速度で更新
		tar_degree  += (tar_ang_vel*180.F/PI)/1000.F;
	...
	*****************************************************************************************/
	if(run_mode == STRAIGHT_MODE){
		if(con_wall.enable == true && sen_fr.value + sen_fl.value <= (TH_SEN_FR+TH_SEN_FL)*5 )		//壁制御が許可されているかチェック
		{
			...
			{
				con_wall.error = 2.0F * (float)(sen_r.error - sen_l.error);	//片方しか使用しないので2倍する
			}
...
/* 制御周期： 250us */
void int_cmt1(void)		//センサ読み込み用割り込み
{
	/*****************************************************************************************
	A/D変換
		センサとバッテリー電圧取得
	...
	*****************************************************************************************/
	...
	if(log_timer % 4 == 0 && log_flag==1 ){
		if(log_timer < (LOG_CNT*4)){
			...
			log[11][log_timer/4] = (int)(I_ang_vel);
			log[12][log_timer/4] = (int)(ang_acc*1000);
		}
	}	
...
void int_cmt2(void)
{
	...
	*****************************************************************************************/	
	if(state == 0){
		...
		p_speed = speed;
		//車体速度を計算
		speed = ((speed_r + speed_l)/2.F);
		
		//I成分のオーバーフローとアンダーフロー対策
		I_speed += speed;
		if(I_speed >30.F*10000000000){
			I_speed = 30.F*10000000000;
	...
	*****************************************************************************************/
	if(state == 1){
		...
		//角速度の更新
		p_ang_vel = ang_vel;
		ang_vel = ((2000.F*(gyro_x)/32767.F))*PI/180.F;
		//積分値の更新
		I_ang_vel += ang_vel;
		if(I_ang_vel >30.F*10000000000){
			I_ang_vel = 30.F*10000000000;
		}else if(I_ang_vel < -1.F*10000000000){
			I_ang_vel = -1.F*10000000000;
		}
		
		//ジャイロの値を角度に変換
		degree += (2.F*(gyro_x_new - gyro_ref)/32767.F);
